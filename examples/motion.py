...
h = 20 # distance to the surface, in mm
...
try:
    while True:
        ...
        vx = h*dx*Apperture/(dt*NofPixels)
        vy = h*dy*Apperture/(dt*NofPixels)
        x += dt*vx
        y += dt*vy
        print("Sensor: dx {:03f} dy {:03f} | Distance (mm): x {:03f} y {:03f} | Quality: {}".format(dx,dy,x,y,q))
